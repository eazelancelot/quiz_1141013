...
public class QuizService {
	...
	/* rollbackFor = Exception.class: 表示只要此方法發生了 Exception， 寫一半的資料都會回朔(rollback)*/
	...
	private BasicRes check(QuizCreateReq req) {
		...
		for(QuestionVo vo : voList) {
			/* 排除非 3種 type*/
			if(!Type.checkType(vo.getType())) { // 前面有驚嘆號，等同於 Type.checkType(vo.getType()) == false
	...
	public BasicRes update(QuizUpdateReq req) throws Exception {
		...
		/* 有找到 quizId 並更新成功(即使要更新的資料與DB中的資料都一樣)，會回傳 1(where 條件帶的是 PK) */
		if(updateRes != 1) {
	...
	public GetQuestionRes getQuestionByQuizId(int quizId) throws Exception {
		...
		/* 把 Question 中的每個字串 options 轉換成自定義的物件 Options*/
